	...
	*/
	...
	html_dropdown = {
		activateCurrent: function (index) {
			this.php_element.find('select').val(index);
		},
		...
		getHtml: function () {
			...
			this.php_element = $('<div class="wrap_controls"></div>').append(title).append(html);

			return this.php_element;
		}
	};
	...
	html_ullist = {
		...
		activateCurrent: function (index) {
			this.php_element.find('li').removeClass('active');
			this.php_element.find('#ullist_a_' + index).parent().addClass('active');
		},
		...
		getHtml: function () {
			...
			if (this.ShowOnMenu(this.view_all_key)) {
				html.append($('<li></li>').append(html_ullist.php_a.call(this, false, this.view_all_key, this.o.view_all_text)));
			}

			for (a = 0; a < this.ln; a++) {
				if (this.ShowOnMenu(a)) {
					html.append($('<li></li>').append(html_ullist.php_a.call(this, a)));
				}
			}
	...
	Maplace = (function () {
		...
		*/
		...
		Maplace.prototype.open_infowindow = function (index, marker) {
			...
			if (point.php && this.type_to_open[type]) {
		...
		Maplace.prototype._init = function () {
			...
			for (var i = 0; i < this.ln; i++) {
				...
				if(this.o.locations[i].php) {
					this.o.locations[i].php = this.o.locations[i].php.replace('%index', i+1);
					this.o.locations[i].php = this.o.locations[i].php.replace('%title', (this.o.locations[i].title || ''));
				}
			}
